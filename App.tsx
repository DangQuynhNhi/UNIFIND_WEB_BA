...
const AppContent: React.FC = () => {
  ...
  const handleSelectChatSession = (sessionId: string) => {
      ...
      if (session) {
          ...
          const displayItem = item || {
              ...
              contactInfo: '', category: ItemCategory.OTHER, status: 'PUBLISHED', userId: 'unknown'
          } as FoundItem;
  ...
  const handleSendFeedback = (text: string) => {
      const msg: Message = { id: Date.now().toString(), senderId: user?.id || 'user', text, timestamp: Date.now() };
        ...
        {!isLoggedIn || !user ? (
            ...
            <>
                ...
                {currentView === 'HOME' && (
                        <>
                        <div
  className="relative overflow-hidden pt-24 pb-32 bg-cover bg-center bg-no-repeat"
  style={{
    backgroundImage: `linear-gradient(rgba(255,255,255,0.45), rgba(255,255,255,0.45)), url('https://i.postimg.cc/SKfbbzgR/Toa-nha-hoc-tap-B1-va-toa-nha-thu-vien-B2-UEL.jpg')`
  }}
                        ...
                        <div className="text-center mb-8 relative z-20 drop-shadow-[0_0_15px_rgba(255,255,255,0.8)]">
                            <h1 className="text-5xl md:text-6xl font-bold tracking-tight drop-shadow-[0_3px_6px_rgba(0,0,0,0.4)]">
                                <span className="text-[#1d4886]">{t.home.heroTitle}</span>{' '}
                                <span className="bg-[linear-gradient(to_right,#f2a900_0%,#d8511d_28%,#8c4a76_65%,#3b76e0_100%)] text-transparent bg-clip-text">
                                    {t.home.heroLost} {t.home.heroFound}
                                </span>
                            </h1>
                        </div>
                        ...
                        <div id="advanced-filters" className="flex flex-wrap items-center gap-3">
                            <div className="relative flex-1 sm:flex-none">
                                ...
                                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as StatusFilterType)} className="w-full sm:w-auto pl-9 pr-8 py-2 bg-brand-800 border border-brand-700 rounded-lg text-sm text-gray-300 focus:outline-none focus:text-white">
                                    ...
                                    <option value="NOT_PROCESSED">{t.common.notProcessed}</option>
                                </select>
                            </div>
                            <div className="relative flex-1 sm:flex-none">
