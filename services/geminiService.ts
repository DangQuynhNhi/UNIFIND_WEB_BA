...
export const validatePostContent = async (title: string, description: string, category: string, imageBase64: string | null): Promise<{ isValid: boolean; errorField: 'title' | 'description' | null; reason: string }> => {
    try {
        const response = await ai.models.generateContent({
            model: 'gemini-3-flash-preview',
            contents: `Analyze: Title "${title}", Desc "${description}". Check for profanity, hate speech, spam. Ensure title is descriptive. Note: Description is optional and can be empty. Return JSON: {isValid: boolean, errorField: string | null, reason: string}`,
            config: {
                responseMimeType: "application/json",
                responseSchema: {
                    type: Type.OBJECT,
