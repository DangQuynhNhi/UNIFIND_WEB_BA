    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-brand-900 border border-brand-700 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-brand-800">
          <h2 className="text-xl font-bold text-white">{t.report.titleFound}</h2>
        ...
        <div className="overflow-y-auto p-6 space-y-4 custom-scrollbar">
          <form id="found-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="border-2 border-dashed border-brand-700 rounded-xl p-8 flex flex-col items-center justify-center text-gray-400 hover:border-brand-500 hover:text-brand-500 transition cursor-pointer bg-brand-800/50">
              ...
              <span className="text-sm">{t.report.uploadPhoto}</span>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">{t.report.itemTitle}</label>
              <input 
                required
                name="title"
                value={formData.title}
                onChange={handleChange}
                placeholder={language === 'vi' ? "VD: Bình nước Hydro Flask Đỏ" : "e.g. Red Hydro Flask"}
            ...
            <div className="grid grid-cols-2 gap-4">
               <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">{t.report.category}</label>
                <select 
                  name="category"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">{t.report.dateFound}</label>
                <input 
                  type="date"
                  name="dateFound"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">{t.report.location}</label>
              <input 
                required
                name="location"
            ...
            <div className="grid grid-cols-2 gap-4">
               <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">{t.report.finderName}</label>
                 <input 
                  required
                  name="finderName"
        ...
        <div className="p-6 border-t border-brand-800 bg-brand-900/50 rounded-b-2xl">
          ...
          >
            {loading ? (
              <>
                <Loader2 className="w-5 h-5 animate-spin" /> {t.report.publishing}
              </>
            ) : (
              t.report.submitFound
            )}
          </button>
        </div>
