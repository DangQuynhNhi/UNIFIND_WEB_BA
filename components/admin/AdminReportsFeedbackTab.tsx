...
export const AdminReportsFeedbackTab: React.FC<AdminReportsFeedbackTabProps> = ({ 
  ...
  const handleDeleteTarget = async (report: Report) => {
      ...
      if (await confirm(t.common.confirm, confirmMsg)) {
          ...
          await alert(language === 'vi' ? "Thành công" : "Success", language === 'vi' ? `Đã xóa ${report.type === 'ITEM' ? 'mục' : 'người dùng'} thành công.` : `Successfully deleted ${report.type === 'ITEM' ? 'item' : 'user'}.`);
      }
  };

  const handleDeleteFeedbackConfirm = async (id: string) => {
      if(await confirm(t.common.confirm, language === 'vi' ? "Xóa tin nhắn phản hồi này?" : "Delete this feedback message?")) {
          onDeleteFeedback(id);
          await alert(language === 'vi' ? "Thành công" : "Success", language === 'vi' ? "Đã xóa tin nhắn phản hồi thành công." : "Successfully deleted feedback message.");
      }
  };
                        ...
                        <div key={fb.id} className="bg-brand-800 border border-brand-700 p-5 rounded-xl shadow-sm hover:bg-brand-800/80 transition flex gap-4 items-start group">
                             ...
                             <div className="flex-1">
                                 <div className="flex justify-between items-center mb-1">
                                    <span className="font-bold text-white text-sm">{fb.senderId === 'user' ? (language === 'vi' ? 'Người dùng ẩn danh' : 'Anonymous User') : fb.senderId}</span>
