        ...
        <div className="flex items-center justify-between p-6 border-b border-brand-800 bg-brand-950/50">
          <div>
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  {language === 'vi' ? 'Chi tiết Vật phẩm' : 'Item Details'}
        ...
        <div className="overflow-y-auto p-6 custom-scrollbar">
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="md:w-1/2">
                    <div className="aspect-video w-full rounded-xl bg-black border border-brand-700 overflow-hidden relative group">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-2 bg-black/60 text-white text-xs text-center">
                            {language === 'vi' ? 'Hình ảnh vật phẩm' : 'Item Image'}
                        </div>
                    </div>
                    ...
                    <div className="mt-4 space-y-3 bg-brand-800/30 p-4 rounded-xl border border-brand-800">
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-300">
                            ...
                            <span className="capitalize">
                                {item.status === 'PUBLISHED' ? (language === 'vi' ? 'Đã đăng' : 'Published') : 
                                 item.status === 'PENDING' ? (language === 'vi' ? 'Chờ duyệt' : 'Pending') : 
                                 item.status === 'COMPLETED' ? t.common.completed :
                                 item.status === 'REJECTED' ? (language === 'vi' ? 'Từ chối' : 'Rejected') :
                                 item.status}
                            </span>
                        </div>
                    </div>
                ...
                <div className="md:w-1/2 space-y-6">
                    ...
                    <div>
                        <h4 className="text-sm font-bold text-gray-400 uppercase mb-2">{language === 'vi' ? 'Thông tin người đăng' : 'Poster Information'}</h4>
                    ...
                    {item.status === 'PENDING' && (
                        <div className="flex items-start gap-2 p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg text-amber-200 text-xs">
                            ...
                            <p>{language === 'vi' ? 'Vật phẩm này đang ở trạng thái Chờ duyệt. Nó chưa hiển thị công khai trên bảng tin.' : 'This item is in Pending status. It is not yet publicly visible on the feed.'}</p>
                        </div>
                    )}
                </div>
            </div>
