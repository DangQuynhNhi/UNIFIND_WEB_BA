...
export const ContactModal: React.FC<ContactModalProps> = ({ 
  ...
  const toggleVoiceInput = () => {
      ...
      if (!SpeechRecognition) {
          alert(language === 'vi' ? "Nhập liệu bằng giọng nói không được hỗ trợ trên trình duyệt này." : "Voice input not supported in this browser.");
          return;
      }
      ...
      if (file && item) {
          ...
          reader.onloadend = async () => {
              ...
              if (result.isValid) {
                  ...
                  setUploadError(result.reason || (language === 'vi' ? "Ảnh bị chặn: Phát hiện nội dung không phù hợp." : "Image blocked: NSFW content detected."));
  ...
  const partnerName = isMePoster ? (language === 'vi' ? "Người liên hệ" : "Student Contact") : item.finderName; 
      ...
      <div className="relative bg-brand-900 border border-brand-700 rounded-2xl w-full max-w-md shadow-2xl flex flex-col h-[600px] max-h-[90vh]">
        ...
        <div className="p-4 border-b border-brand-800 bg-brand-950/50 flex justify-between items-center rounded-t-2xl">
            <div 
                ...
                title={language === 'vi' ? 'Xem hồ sơ & Báo cáo' : "View Profile & Report"}
            >
            ...
            <div className="flex items-center gap-1">
                ...
                {isItemCompleted ? (
                    ...
                    <button 
                        ...
                        title={language === 'vi' ? 'Nhấn khi vật phẩm đã được trả lại' : "Click when item is returned"}
                    >
                        {isMyTurnConfirmed 
            ...
            {isSending && (
                 <div className="flex justify-end">
                    <div className="bg-brand-600/50 text-white/50 rounded-2xl rounded-tr-none px-4 py-2 text-sm flex items-center gap-2">
                        <Loader2 className="w-3 h-3 animate-spin"/> {language === 'vi' ? 'đang gửi...' : 'sending...'}
                    </div>
                 </div>
            )}
            ...
            <div className="flex items-end gap-2">
                <div className="flex gap-1 mb-1">
                    ...
                    <button 
                        ...
                        title={language === 'vi' ? 'Gửi ảnh' : "Send Image"}
                    >
                    ...
                    <button 
                        ...
                        title={language === 'vi' ? 'Nhập bằng giọng nói' : "Voice Input"}
                    >
