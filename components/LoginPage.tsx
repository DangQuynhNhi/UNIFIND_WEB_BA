  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
        ...
        if (!email.includes('@st.uel.edu.vn') && !email.includes('@student.edu.vn')) {
             setError(language === 'vi' ? 'Email phải là địa chỉ trường hợp lệ (@st.uel.edu.vn)' : 'Email must be a valid school address (@st.uel.edu.vn)');
             setIsLoading(false);
             return;
        }
        ...
        if (!user) {
            setError(language === 'vi' ? 'Không tìm thấy người dùng. Vui lòng liên hệ quản trị viên.' : 'User not found. Please contact administration.');
            setIsLoading(false);
            return;
        }

        if (user.isLocked) {
            setError(language === 'vi' ? 'Tài khoản này đã bị khóa do hoạt động nghi ngờ.' : 'This account has been locked due to suspicious activity.');
            setIsLoading(false);
            return;
        }
        ...
        if (password !== user.studentId) {
            setError(language === 'vi' ? 'Mật khẩu không chính xác. Mật khẩu mặc định là MSSV của bạn.' : 'Incorrect password. Default password is your Student ID.');
            setIsLoading(false);
            return;
        }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/unauthorized-domain') {
        ...
        setError(err.message || (language === 'vi' ? 'Đăng nhập bằng Google thất bại' : 'Failed to login with Google'));
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleForgotPassword = (e: React.FormEvent) => {
      ...
      setTimeout(() => {
          if (!forgotEmail.includes('@st.uel.edu.vn') && !forgotEmail.includes('@student.edu.vn')) {
              setError(language === 'vi' ? 'Vui lòng nhập địa chỉ email trường hợp lệ.' : 'Please enter a valid school email address.');
              setIsLoading(false);
              return;
          }
       ...
       <div className="w-full max-w-md bg-brand-800/80 backdrop-blur-md border border-brand-700 rounded-2xl shadow-2xl overflow-hidden z-10">
                ...
                <form onSubmit={handleLogin} className="p-8 space-y-6">
                        ...
                        <div className="bg-red-500/10 border border-red-500/50 rounded-lg p-3 text-red-700 bg-red-50 font-medium text-sm flex flex-col gap-2 animate-fade-in">
                            ...
                            {error.includes('Authorized domains') && (
                                <div className="mt-2 pt-2 border-t border-red-200 space-y-2">
                                    ...
                                    >
                                        {language === 'vi' ? 'Mở Cài đặt Firebase Console' : 'Open Firebase Console Settings'}
                                    </a>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative group">
                            ...
                            <input 
                                ...
                                placeholder={language === 'vi' ? 'Nhập MSSV của bạn' : 'Enter your MSSV'}
                    ...
                    >
                     ...
                     <div className="mt-4 text-center">
                        <p className="text-xs text-gray-500">{language === 'vi' ? 'Để thử luồng đăng nhập lần đầu, hãy thử:' : 'For testing first-login flow, try:'}</p>
                    ...
                    {!resetSent ? (
                        <form onSubmit={handleForgotPassword} className="space-y-6">
                            ...
                            <div className="bg-brand-900/50 p-4 rounded-lg border border-brand-700 mb-4">
                                <p className="text-sm text-gray-300">
                                    {language === 'vi' 
                                      ? 'Nhập địa chỉ email sinh viên của bạn. Chúng tôi sẽ gửi cho bạn một liên kết xác minh để đặt lại mật khẩu.' 
                        ...
                        <div className="text-center space-y-4 animate-fade-in">
                            ...
                            <div className="p-3 bg-brand-900/50 rounded border border-brand-700 text-xs text-gray-400">
                                {language === 'vi' 
                                  ? 'Hãy nhớ kiểm tra thư mục thư rác nếu bạn không thấy nó trong vòng vài phút.' 
                    ...
                    <div className="space-y-3">
                        ...
                        >
                         ...
                         <p className="text-[10px] text-gray-500 italic">
                            {language === 'vi' ? '(Mô phỏng: Nhấp vào đây sẽ xác thực bạn)' : '(Simulation: Clicking this will authenticate you)'}
                        </p>
                    </div>
            ...
            <div className="bg-brand-900/50 p-4 text-center border-t border-brand-700">
                <p className="text-xs text-gray-500">
                    ...
                    {language === 'vi' ? 'Liên hệ Phòng Đào tạo nếu bạn không thể đăng nhập.' : 'Contact Academic Affairs if you cannot login.'}
                </p>
            </div>
       </div>
    </div>
