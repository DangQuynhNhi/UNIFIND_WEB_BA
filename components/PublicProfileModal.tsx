        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <div className="relative bg-brand-900 w-full max-w-6xl h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
                ...
                <div className="flex items-center justify-between p-4 border-b border-brand-800 bg-brand-950">
                    ...
                    >
                        <ArrowLeft className="w-5 h-5"/> {language === 'vi' ? 'Quay lại' : 'Back to Summary'}
                    </button>
                    <span className="font-bold text-white text-lg">
                        {language === 'vi' ? `Đang xem hồ sơ của ${user.name}` : `Viewing ${user.name}'s Profile`}
                    </span>
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      ...
      <div className="relative bg-brand-900 border border-brand-700 rounded-2xl w-full max-w-sm shadow-2xl flex flex-col animate-fade-in overflow-hidden">
        ...
        <div className="px-6 pb-6 -mt-12 flex flex-col items-center">
             ...
             <span className="px-2 py-0.5 bg-brand-800 text-brand-400 text-xs rounded border border-brand-700">
             ...
             <div className="w-full mt-6 space-y-3">
                 <div className="flex items-center gap-3 p-3 bg-brand-950/50 rounded-lg border border-brand-800">
                     ...
                     <div className="text-sm">
                         <p className="text-white font-medium">{language === 'vi' ? 'Sinh viên đã xác thực' : 'Verified Student'}</p>
                         <p className="text-gray-500 text-xs">{language === 'vi' ? 'Email đã được xác nhận qua ID trường' : 'Email confirmed via School ID'}</p>
                     </div>
                 </div>
                 ...
                 >
                     <Eye className="w-4 h-4"/> {language === 'vi' ? 'Xem chi tiết hồ sơ' : 'View Full Profile'}
                 </button>
                 ...
                 {currentUserRole !== 'ADMIN' && (
                     ...
                     >
                         <Flag className="w-4 h-4"/> {language === 'vi' ? 'Báo cáo người dùng' : 'Report User'}
                     </button>
                 )}
             </div>
        </div>
