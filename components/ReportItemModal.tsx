...
export const ReportItemModal: React.FC<ReportItemModalProps> = ({ isOpen, onClose, onSubmit, initialType }) => {
  ...
  }, [formData.title, formData.description, formData.location, formData.category, formData.dateFound]);
  ...
  const handleImageValidation = async (base64Data: string) => {
    ...
    if (result.isValid) {
        ...
        setImageError(result.reason || (language === 'vi' ? "Hình ảnh chứa nội dung không phù hợp." : "Image contains inappropriate content."));
        return false;
    }
  };
      ...
      if (isMapOpen && !mapRef.current) {
          setTimeout(() => {
            if (document.getElementById('leaflet-map')) {
                // Default coordinates for "669 Đỗ Mười, khu phố 13, Thủ Đức, Thành phố Hồ Chí Minh" (UEL)
                const defaultLat = 10.8702;
                const defaultLng = 106.7782;
                
                const map = L.map('leaflet-map').setView([defaultLat, defaultLng], 16);
                
                L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                    attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                }).addTo(map);

                const initialLat = mapCoords?.lat || defaultLat;
                const initialLng = mapCoords?.lng || defaultLng;
  ...
  const handleGetCurrentLocation = () => {
    if (!navigator.geolocation) {
        alert(language === 'vi' ? "Trình duyệt của bạn không hỗ trợ định vị" : "Geolocation is not supported by your browser");
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        async (position) => {
            ...
            setFormData(prev => ({ ...prev, location: language === 'vi' ? `Đang lấy địa chỉ...` : `Fetching address...` }));
            const address = await reverseGeocode(latitude, longitude);
            setFormData(prev => ({ ...prev, location: address }));
            setGettingLocation(false);
        },
        (error) => {
            alert(language === 'vi' ? "Lỗi định vị." : "Location error.");
            setGettingLocation(false);
        },
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (!file.type.startsWith('image/')) {
        alert(language === 'vi' ? 'Vui lòng tải lên tệp hình ảnh' : 'Please upload an image file');
        return;
      }
  ...
  const handleAiGenerate = async () => {
    ...
    const contextTitle = `${type === 'LOST' ? (language === 'vi' ? 'Vật phẩm bị mất:' : 'Lost Item:') : (language === 'vi' ? 'Vật phẩm tìm thấy:' : 'Found Item:')} ${formData.title}`;
              ...
              <div className="bg-brand-900 rounded-xl border border-brand-700 w-full max-w-3xl h-[80vh] flex flex-col shadow-2xl overflow-hidden">
                  <div className="p-4 border-b border-brand-700 flex justify-between items-center bg-brand-800">
                      <div>
                        <h3 className="text-white font-bold flex items-center gap-2">
                            ...
                            {language === 'vi' ? 'Điều chỉnh vị trí' : 'Adjust Location'}
                        </h3>
                        <p className="text-xs text-gray-400">{language === 'vi' ? 'Kéo dấu màu xanh đến vị trí chính xác.' : 'Drag the blue marker to the exact spot.'}</p>
                      </div>
                      <button onClick={() => {
                          setIsMapOpen(false);
                          if(mapRef.current) { mapRef.current.remove(); mapRef.current = null; }
                  ...
                  <div className="flex-1 bg-slate-100 relative">
                       ...
                       <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[400] flex gap-2">
                           ...
                           >
                               <CheckCircle2 className="w-4 h-4"/> {language === 'vi' ? 'Xác nhận vị trí' : 'Confirm Location'}
                           </button>
                       </div>
                  </div>
              </div>
          ...
          <form id="report-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input 
                ...
                placeholder={isLost ? (language === 'vi' ? "VD: MacBook Pro Bạc" : "e.g. Silver MacBook Pro") : (language === 'vi' ? "VD: Bình nước Hydro Flask Đỏ" : "e.g. Red Hydro Flask")}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">{isLost ? t.report.dateLost : t.report.dateFound} <span className="text-red-500">*</span></label>
                <input 
                  required
                  type="date"
                  name="dateFound"
            ...
            <div>
              ...
              <div className="relative">
                  <input 
                    ...
                    onChange={handleChange}
                    placeholder={language === 'vi' ? "VD: Thư viện, Bàn 4" : "e.g. Library, Table 4"}
                  ...
                  <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                      <button type="button" onClick={() => setIsMapOpen(true)} className="p-1.5 text-brand-500 hover:bg-brand-100 rounded-lg transition" title={language === 'vi' ? 'Chọn trên bản đồ' : "Pick on Map"}>
                          <MapIcon className="w-4 h-4" />
                      </button>
                      <button type="button" onClick={handleGetCurrentLocation} disabled={gettingLocation} className={`p-1.5 rounded-lg transition flex items-center gap-1 ${gettingLocation ? 'text-brand-accent animate-pulse' : 'text-gray-400 hover:text-brand-600 hover:bg-brand-100'}`} title={language === 'vi' ? 'Lấy vị trí hiện tại' : "Get Current Location"}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">{t.report.contact}</label>
                <input 
                  name="contactInfo"
